...
public class UICamera : MonoBehaviour
{
    ...
    public class MouseOrTouch
    {
        ...
        /// <summary>上次点击响应按钮点击成功的时间</summary>
        public float clickUsedTime = 0f;
    ...
    /// <summary>
    /// 是否使用按钮点击间隔设置，间隔超0.5秒才响应,注意！打开之后双击功能失效！
    /// </summary>
    public static bool useButtonClickGapTime = false;
    ...
    private GameObject lastClickGO = null;
        ...
        if (currentTouch.pressed != null)
        {
                ...
                currentTouch.totalDelta.sqrMagnitude < drag))
            {
                ...
                if (currentTouch.clickNotification != ClickNotification.None && currentTouch.pressed == currentTouch.current)
                {
                    ShowTooltip(null);
                    float time = RealTime.time;
                    if (!useButtonClickGapTime || lastClickGO == null || lastClickGO != currentTouch.pressed || (useButtonClickGapTime && lastClickGO == currentTouch.pressed && lastClickTime + 0.5f <= time))
                    {
                        if (onClick != null) onClick(currentTouch.pressed);
                        Notify(currentTouch.pressed, "OnClick", null);
                        lastClickTime = time;
                        lastClickGO = currentTouch.pressed;
