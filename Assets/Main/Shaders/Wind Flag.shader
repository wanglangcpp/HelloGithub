// 本 Shader 用于需要与旗帜类似的物体，通过改变顶点实现，对材质所对应的模型有特别要求。
// 当前 Shader 使用的是 ShadowGun 原本的旗帜模型，随后若有改动会在此处列出。

Shader "Game/Wind Flag" {
	...
	SubShader {
		...
		inline float4 AnimateVertex2(half4 pos, half3 normal, half4 animParams,half4 wind,half2 time) {
			...
			half fObjPhase = dot(_Object2World[3].xyz, 1);
