...
namespace Genesis.GameClient.Behavior
{
    ...
    public class ContainHeroesWithElements : Conditional
    {
        [SerializeField]
        ...
        [SerializeField]
        private int[] m_ElementIds = null;

        public override void OnStart()
        {
            if (m_ElementIds == null)
            {
                m_ElementIds = new int[0];
            }
        }
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            for (int i = 0; i < m_ElementIds.Length; ++i)
            {
                candidateProfessionSet.Add(m_ElementIds[i]);
            }

            int count = 0;
            for (int i = 0; i < meHeroes.Length; ++i)
            {
                if (candidateProfessionSet.Contains(meHeroes[i].ElementId))
                {
                    ++count;
